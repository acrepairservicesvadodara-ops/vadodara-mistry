...
// Data copied from lib/data.ts (we'll parse it directly)
...
// Extract keyword arrays with category info
...
const areaKeywords = extractArray(dataContent, "areaKeywords").map((a) => ({
  ...a,
  areaName: a.title, // the regex captures it as title
}));
const vadodaraAreas = extractArray(dataContent, "vadodaraAreas").map((a) => ({
  slug: a.slug,
  name: a.title, // the regex captures it as title
}));
...
console.log(`  Painting keywords: ${paintingKeywords.length}`);
console.log(`  Civil keywords: ${civilKeywords.length}`);
console.log(`  Plumbing keywords: ${plumbingKeywords.length}`);
console.log(`  Carpenter keywords: ${carpenterKeywords.length}`);
console.log(`  Fabrication keywords: ${fabricationKeywords.length}`);
console.log(`  Solar keywords: ${solarKeywords.length}`);
console.log(`  All keywords total: ${allKeywords.length}`);
console.log(`  Area keywords: ${areaKeywords.length}`);
console.log(`  Vadodara areas: ${vadodaraAreas.length}`);
...
export const metadata: Metadata = {
  ...
  description: \`Looking for ${title.toLowerCase()} in Vadodara? Vadodara Mistry offers professional ${description.toLowerCase()}. ✓ Experienced Team ✓ Best Prices ✓ Quality Work. Call +91 93139 82980\`,
  keywords: [
    "${title}",
    "${title} Vadodara",
    "${title} services",
  ...
  openGraph: {
    ...
    description: \`Professional ${title.toLowerCase()} services in Vadodara.\`,
      ...
      <section className={\`\${config.gradient} py-8 md:py-20\`}>
        <div className="container mx-auto px-4">
          ...
          <div className="md:hidden mb-6">
            <BookingForm title={\`Book ${title}\`} compact />
          </div>
          ...
          <div className="max-w-4xl mx-auto text-center text-white">
            ...
            <p className="text-lg md:text-xl mb-6 md:mb-8 text-white/90">
              ${description} in Vadodara. Trusted by 10,000+ customers. 
...
export const metadata: Metadata = {
  title: \`Home Services in ${areaName}, Vadodara | Painter, Plumber, Carpenter\`,
  description: \`Best home services in ${areaName}, Vadodara. Professional painters, plumbers, carpenters, civil contractors & fabrication work. Call +91 93139 82980\`,
  keywords: \`home services ${areaName.toLowerCase()} vadodara, painter ${areaName.toLowerCase()}, plumber ${areaName.toLowerCase()}\`,
  openGraph: {
    title: \`Home Services in ${areaName}, Vadodara | Vadodara Mistry\`,
    description: \`Best home services in ${areaName}, Vadodara.\`,
    url: "https://vadodaramistry.com/${slug}",
    type: "website",
  },
};
...
// Helper to convert slug to PascalCase for component name
...
for (const keyword of allKeywords) {
  ...
  if (fs.existsSync(folderPath)) {
    console.log(`Skipping (exists): ${keyword.slug}`);
    skipped++;
    continue;
  }
...
for (const area of areaKeywords) {
  ...
  if (fs.existsSync(folderPath)) {
    console.log(`Skipping (exists): ${area.slug}`);
    skipped++;
    continue;
  }
